    ...
    <div>
        <div class="crumbs">
            <el-breadcrumb separator="/">
                <el-breadcrumb-item> <i class="el-icon-lx-cascades"></i> 潜在客户管理 </el-breadcrumb-item>
            </el-breadcrumb>
        </div>
        <div class="container">
            ...
            <el-table :data="tableData" border class="table" ref="multipleTable" header-cell-class-name="table-header">
                ...
                <el-table-column label="操作" align="center" width="300">
                    <template slot-scope="scope">
                        <el-button type="warning" size="mini" @click="shenpi(scope.row.id)">转为客户</el-button>
        ...
        <!-- 编辑弹出框 -->
        <el-dialog title="潜在客户编辑" :visible.sync="editVisible" width="30%">
            <el-form ref="ruleForm" :model="eform" :rules="rules" label-width="90px">
                ...
                <el-form-item label="市场活动" prop="aid">
                    <el-select v-model="eform.aid" placeholder="请选择市场活动" clearable="true">
                        <el-option v-for="item in activitys" :key="item.id" :label="item.title" :value="item.id"> </el-option>
                    </el-select>
                </el-form-item>
            </el-form>
        ...
        <!-- 添加弹出框 -->
        <el-dialog title="潜在客户添加" :visible.sync="addVisible" width="30%">
            <el-form ref="ruleForm" :model="aform" :rules="rules" label-width="90px">
                ...
                <el-form-item label="市场活动" prop="aid">
                    <el-select v-model="aform.aid" placeholder="请选择市场活动" clearable="true">
                        ...
                        <!-- <template v-if="activitys.status === '03' ||activitys.status==='02'">
                            <el-option v-for="item in activitys"  :key="item.id" :label="item.title" :value="item.id"></el-option>
                        </template> -->
                    </el-select>
                </el-form-item>
...
export default {
    data() {
        ...
        return {
            ...
            totalCount: 0,
            rules: {
                name: [{ required: true, message: '请输入潜在客户名称', trigger: 'blur' }],
                phone: [
                ...
                type: [{ required: true, message: '请输入潜在客户类型', trigger: 'change' }],
                aid: [{ required: true, message: '请输入市场活动', trigger: 'change' }]
            },
            activitys: []
        };
    },
    ...
    methods: {
        ...
        getActivity() {
            this.$axios.post('/api/activity/selectByCondition', {}).then((res) => {
                ...
                {
                    if(res.data[i]['status']=='03'|| res.data[i]['status']=='02')
                    {
